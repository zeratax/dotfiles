...
export PATH=":~/bin:/usr/local/sbin:/usr/local/bin:/usr/bin:/usr/bin/site_perl:/usr/bin/vendor_perl:/usr/bin/core_perl:~/.gem/ruby/2.3.0/bin"
...
svg2pdf() { if [ $# -eq 0 ]; then echo "No arguments specified. "; return 1; fi
  filename=$(basename "$1")
  filename="${filename%.*}"
